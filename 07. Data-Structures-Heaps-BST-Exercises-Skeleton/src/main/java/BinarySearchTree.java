...
public class BinarySearchTree<E extends Comparable<E>> {
    ...
    public void eachInOrder(Consumer<E> consumer) {
        nodeInOder(this.root, consumer);
    }

    private void nodeInOder(Node<E> node, Consumer<E> consumer) {
        if (node == null) {
            return;
        }
        nodeInOder(node.getLeft(), consumer);
        consumer.accept(node.getValue());
        nodeInOder(node.getRight(), consumer);
    ...
    public boolean contains(E element) {
        // recursive: return containsNode(this.root,element)!=null;
